package org.zcj.rpc.server.codec;
 ...
 * Description:
 */
public class RpcServerEncoder extends MessageToByteEncoder<Protocol> {

    private Logger log = LoggerFactory.getLogger(RpcServerEncoder.class);

    @Override
    protected void encode(ChannelHandlerContext ctx, Protocol protocol, ByteBuf out) throws Exception {
        log.debug("send request to client : " + protocol);
